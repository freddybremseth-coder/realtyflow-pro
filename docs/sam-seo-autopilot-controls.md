...
A live OPTIONS check also identified the immediate collector blocker: `/api/public/search-discovery` returned HTTP 307 to `/login` for all eight portfolio origins. The middleware now exempts this exact path (not its children or other public/admin APIs). The handler still enforces its explicit origin allowlist. Ten middleware/collector tests pass, including all eight allowed origins, untrusted origins, and private sibling/child routes. This fixes admission in code; production must be deployed and rechecked before claiming arrivals are flowing. No test visits were inserted.
