...
## Implementation update: narrow staff edits of owner-approved joint contacts (draft only)

The proposed Zen-only `crm.joint.write` permission is **distinct** from generic `crm.write` and requires the employee to have `crm.joint.read` as well. The focused Zen staff CRM may edit only the name, email and phone number of an already owner-approved eligible joint customer, never create a Zen record through an unrestricted import, change sale status, commission, lead eligibility, brand, notes, documents, tasks, marketing accounts or any historical client's record. Generic Zen staff CRM endpoints stay blocked.

`PATCH /api/workspaces/zeneco/joint-contacts` requires a current signed employee session, live verified workspace grant and verified Auth ID. The service-only `workspace_zeneco_joint_contact_update` RPC re-checks in the same database UPDATE that BOTH Zen CRM brand columns still match, the contact was created after the cutoff, the original first enquiry was independently documented and owner-approved, and the current employee has BOTH joint permissions. It records an actor/contact/changed-fields-only audit entry, with no PII copied to that audit table. An owner revocation or removed membership prevents subsequent edits; no generic create or delete is opened. No email/phone deduplication against private other-brand customer lists is exposed to staff.

The updated allowance is not yet in use: the new migration is a staging-only draft, feature flag remains OFF, no employee has been invited, no real customer has been approved, and no payouts or commission ledger transactions are made. A comprehensive staging database integration run, cross-brand checks, revocation race tests and direct legacy-route security review are still blockers before real access can be activated.
