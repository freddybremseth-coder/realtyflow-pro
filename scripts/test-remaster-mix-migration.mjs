...
async function main() {
  const client = new Client({ connectionString: databaseUrl(), application_name: "remaster_mix_migration_test" });
  ...
  try {
    ...
    const table = await client.query(`select to_regclass('public.remaster_mix_jobs') as name`);
    assert(table.rows[0]?.name === "remaster_mix_jobs", "remaster_mix_jobs table was not created.");

    const privileges = await client.query(`
      select
        has_function_privilege('anon', 'public.claim_remaster_mix_job(text,integer)', 'execute') as anon_claim,
        has_function_privilege('authenticated', 'public.claim_remaster_mix_job(text,integer)', 'execute') as authenticated_claim,
        has_function_privilege('service_role', 'public.claim_remaster_mix_job(text,integer)', 'execute') as service_claim
    `);
    assert(privileges.rows[0]?.anon_claim === false, "anon must not execute claim_remaster_mix_job.");
    assert(privileges.rows[0]?.authenticated_claim === false, "authenticated must not execute claim_remaster_mix_job.");
    assert(privileges.rows[0]?.service_claim === true, "service_role must execute claim_remaster_mix_job.");
    ...
    const inserted = await client.query(`
      insert into public.remaster_mix_jobs (
        ...
        'Mediterranean Sunset Mix #001', 'mediterranean-sunset', 120, 8,
        'Mediterranean Sunset Deep House', true, 'north', 'villas', 20,
        'Explore Costa Blanca at ZenEcoHomes.com', array['track-a','track-b'],
        'queued', 'queued', now()
      ) returning id
    `);
    ...
    const claim1 = await client.query(`select * from public.claim_remaster_mix_job($1, $2)`, ["worker-a", 300]);
    ...
    const claimWhileRunning = await client.query(`select * from public.claim_remaster_mix_job($1, $2)`, ["worker-b", 300]);
    assert(claimWhileRunning.rowCount === 0, "A leased running job must not be claimed by another worker.");
    ...
    assert(heartbeat.rows[0].pipeline_step === "rendering_visuals", "Heartbeat must update pipeline step.");
    ...
    assert(failed.rows[0].status === "queued", "Retryable pre-upload failure must return the job to queued.");
    assert(Number(failed.rows[0].retry_count) === 1, "Retryable failure must increment retry_count.");

    const claim2 = await client.query(`select * from public.claim_remaster_mix_job($1, $2)`, ["worker-c", 300]);
    ...
    assert(completed.rows[0].status === "completed", "Verified YouTube result must complete the job.");
    assert(Number(completed.rows[0].progress) === 100, "Completed mix must have progress 100.");
    assert(completed.rows[0].youtube_video_id === "youtube-test-id", "Completed mix must persist YouTube id.");
    assert(completed.rows[0].lease_token === null, "Completed mix must clear the lease token.");

    const noMoreWork = await client.query(`select * from public.claim_remaster_mix_job($1, $2)`, ["worker-d", 300]);
