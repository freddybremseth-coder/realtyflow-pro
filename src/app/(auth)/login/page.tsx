...
export default function LoginPage() {
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    if (!res.ok) setError(data.error || "Kunne ikke sende lenke for nytt passord.");
    else setMessage("Hvis e-posten er godkjent, får du straks en lenke for nytt passord.");
    setLoading(false);
  };
      ...
      <Card className="w-full max-w-md mx-4">
        ...
        <CardContent>
          <form onSubmit={resetMode ? handleResetPassword : handleLogin} className="space-y-4">
            ...
            <Button type="submit" className="w-full" disabled={loading}>{loading ? "Vennligst vent..." : resetMode ? "Send lenke" : "Logg inn"}</Button>
            <button type="button" onClick={() => { setResetMode(!resetMode); setError(""); setMessage(""); }} className="w-full text-sm text-slate-400 hover:text-primary-400 transition-colors">{resetMode ? "Tilbake til innlogging" : "Glemt passord?"}</button>
            <p className="flex items-center justify-center gap-2 text-xs text-slate-500"><ShieldCheck size={14}/> Tilgang styres av Owner og signerte roller.</p>
          </form>
        </CardContent>
      </Card>
    </div>
