  ...
  return <div style={{ maxWidth: 1500, margin: "0 auto", padding: 24, fontFamily: "system-ui, sans-serif" }}>
    ...
    {s && <>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 10, marginTop: 18 }}>
        ...
        <Metric label="Approved" value={s.approvedRecommendations} sub="Ikke applied" />
      ...
      <section style={{ marginTop: 18, padding: 14, borderRadius: 12, background: "#f8fafc", border: "1px solid #e2e8f0" }}>
        ...
        <div style={{ marginTop: 5, color: "#475569", fontSize: 13 }}>AI analyserer og foreslår. <b>Godkjenn</b> endrer bare anbefalingens status til approved. Det utfører ikke KDP-, Ads-, Amazon- eller website-endringer. <b>Applied</b> er en separat tilstand og skal bare settes av en senere, eksplisitt apply-handling med audit trail.</div>
      </section>
      ...
      <section style={{ marginTop: 18, border: "1px solid #e2e8f0", borderRadius: 12, background: "white", overflow: "hidden" }}>
        <div style={{ padding: 14, borderBottom: "1px solid #e2e8f0" }}>
          ...
          <div style={{ marginTop: 4, fontSize: 12, color: "#64748b" }}>Score = faktisk web-intent siste 30 dager + readiness gaps + pending-signaler. Book Report og Amazon Ads legges inn som egne økonomiske signaler senere.</div>
        </div>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            <thead><tr style={{ background: "#f8fafc", textAlign: "left" }}>
              {['#','Bok','Score','Intent','Gap','ASIN','Work','Views','Sample','Amazon','Pending'].map((h) => <th key={h} style={{ padding: 9, borderBottom: "1px solid #e2e8f0", whiteSpace: "nowrap" }}>{h}</th>)}
            </tr></thead>
            <tbody>{(data?.priority ?? []).slice(0, 20).map((b, i) => <tr key={b.bookId}>
              ...
              <td style={{ padding: 9, borderBottom: "1px solid #f1f5f9", minWidth: 220 }}><b>{b.title}</b><div style={{ color: "#94a3b8", marginTop: 2 }}>{b.seriesTitle ?? "Standalone"}{b.seriesNumber ? ` · #${b.seriesNumber}` : ""} · {b.language ?? "—"}</div></td>
              <td style={{ padding: 9, borderBottom: "1px solid #f1f5f9", fontWeight: 900 }}>{b.score}</td>
              <td style={{ padding: 9, borderBottom: "1px solid #f1f5f9" }}>{b.scoreComponents.intentScore}</td>
              <td style={{ padding: 9, borderBottom: "1px solid #f1f5f9" }}>{b.scoreComponents.readinessGap}</td>
              <td style={{ padding: 9, borderBottom: "1px solid #f1f5f9", whiteSpace: "nowrap" }}>{b.asin ?? "Mangler"}</td>
              <td style={{ padding: 9, borderBottom: "1px solid #f1f5f9" }}>{b.workId ? (b.workVerified ? "✓ verified" : b.workStatus ?? "linked") : "Mangler"}</td>
              <td style={{ padding: 9, borderBottom: "1px solid #f1f5f9" }}>{b.events30d.bookViews}</td>
              <td style={{ padding: 9, borderBottom: "1px solid #f1f5f9" }}>{b.events30d.sampleClicks}</td>
              <td style={{ padding: 9, borderBottom: "1px solid #f1f5f9" }}>{b.events30d.amazonClicks}</td>
              <td style={{ padding: 9, borderBottom: "1px solid #f1f5f9" }}>{b.pendingRecommendations}</td>
            </tr>)}</tbody>
          </table>
        </div>
      </section>
