...
type Data = { available: boolean; summary: any; facts: any[]; batches: any[]; exceptions: any[]; editions: any[] };
...
export default function SalesEvidencePage() {
  ...
  const [busyExceptionId, setBusyExceptionId] = useState("");
  ...
  async function resolutionAction(exceptionId: string, action: "stage_resolution" | "decide_resolution", editionId?: string, resolutionId?: string, decision?: "approve" | "reject") {
    setBusyExceptionId(exceptionId); setError(""); setNotice("");
    try {
      const payload = action === "stage_resolution"
        ? { action, exceptionId, editionId }
        : { action, resolutionId, decision, note: decisionNotes[exceptionId] || undefined };
      const response = await fetch("/api/book-growth/sales-evidence", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error || "Avviksbehandlingen mislyktes");
      setNotice(action === "stage_resolution" ? "Koblingen er foreslått, men ingen salgsrad er opprettet. Den venter på separat godkjenning." : decision === "approve" ? "Koblingen er godkjent og én sporbar kanonisk salgsrad er opprettet." : "Forslaget er avvist. Ingen salgsrad ble opprettet.");
      setDecisionNotes((current) => ({ ...current, [exceptionId]: "" }));
      await load();
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Avviksbehandlingen mislyktes"); }
    finally { setBusyExceptionId(""); }
  }
  ...
  return <main style={{ maxWidth: 1400, margin: "0 auto", padding: 24, fontFamily: "system-ui,sans-serif" }}>
    <header style={{ display: "flex", justifyContent: "space-between", gap: 16, alignItems: "flex-start", flexWrap: "wrap" }}>
      <div><p style={{ margin: 0, color: "#7c3aed", fontWeight: 900 }}>BOOK OS · FASE 5.1</p><h1 style={{ margin: "5px 0" }}>Sales Evidence Center</h1><p style={{ maxWidth: 850, color: "#475569", marginTop: 0 }}>Knytter salgs-, royalty- og annonsetall til riktig bok, utgave og kanonisk revisjon. Umatchede rader krever forslag og separat godkjenning før de blir kanoniske salgsbevis.</p></div>
    ...
    <section style={{ marginTop: 16, padding: 14, border: "1px solid #f59e0b", borderRadius: 12, background: "#fffbeb" }}><h2 style={{ marginTop: 0 }}>Avvik som må løses</h2><p style={{ color: "#92400e", fontSize: 12 }}>Velg riktig utgave og opprett først et forslag. Forslaget må godkjennes i et separat steg.</p>{(data?.exceptions ?? []).map((row) => {
      const pending = row.resolutions?.find((resolution: any) => resolution.status === "pending");
      const proposedEdition = pending ? data?.editions?.find((edition) => edition.id === pending.edition_id) : null;
      const itemBusy = busyExceptionId === row.id;
      return <article key={row.id} style={{ padding: 11, marginTop: 9, border: "1px solid #fde68a", borderRadius: 8, background: "white", fontSize: 12 }}><strong>{row.reason === "book_missing" ? "Boken finnes ikke i eldre katalog" : "Kanonisk utgave mangler"}</strong><span style={{ display: "block", color: "#92400e", margin: "4px 0 8px" }}>{row.evidence?.channel || "ukjent kanal"} · {row.evidence?.marketplace || "global"} · {row.evidence?.format || "ukjent format"} · {row.evidence?.metric_date || "ukjent dato"}</span>{pending ? <div style={{ display: "grid", gap: 7, padding: 9, background: "#eff6ff", borderRadius: 7 }}><strong style={{ color: "#1d4ed8" }}>Foreslått: {proposedEdition?.work?.canonical_title || proposedEdition?.title || "Ukjent utgave"} · {proposedEdition?.language?.toUpperCase()} · {proposedEdition?.format}</strong><span>Venter separat godkjenning · ingen salgsrad opprettet</span><button disabled={itemBusy} onClick={() => resolutionAction(row.id,"decide_resolution",undefined,pending.id,"approve")} style={{ padding: 8, border: 0, borderRadius: 6, background: "#166534", color: "white", fontWeight: 900 }}>{itemBusy ? "Godkjenner…" : "Godkjenn kobling"}</button><input value={decisionNotes[row.id] || ""} onChange={(event) => setDecisionNotes((current) => ({ ...current, [row.id]: event.target.value }))} placeholder="Begrunnelse for avvisning" style={{ padding: 7, border: "1px solid #94a3b8", borderRadius: 6 }} /><button disabled={itemBusy || !(decisionNotes[row.id] || "").trim()} onClick={() => resolutionAction(row.id,"decide_resolution",undefined,pending.id,"reject")} style={{ padding: 7, border: "1px solid #b45309", borderRadius: 6, background: "white", color: "#92400e", fontWeight: 800 }}>Avvis forslag</button></div> : <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}><select value={selectedEditions[row.id] || ""} onChange={(event) => setSelectedEditions((current) => ({ ...current, [row.id]: event.target.value }))} style={{ flex: "1 1 320px", padding: 8, border: "1px solid #94a3b8", borderRadius: 6 }}><option value="">Velg kanonisk utgave…</option>{(data?.editions ?? []).map((edition) => <option key={edition.id} value={edition.id}>{edition.work?.canonical_title || edition.title} · {edition.language?.toUpperCase()} · {edition.format}</option>)}</select><button disabled={itemBusy || !selectedEditions[row.id]} onClick={() => resolutionAction(row.id,"stage_resolution",selectedEditions[row.id])} style={{ padding: 8, border: 0, borderRadius: 6, background: "#1d4ed8", color: "white", fontWeight: 900 }}>{itemBusy ? "Oppretter…" : "Foreslå kobling"}</button></div>}</article>;
    })}{!data?.exceptions?.length ? <p style={{ color: "#166534" }}>Ingen åpne avvik.</p> : null}</section>
