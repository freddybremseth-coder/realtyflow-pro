...
export default function BrandBrainPage() {
  ...
  return <main className="mx-auto max-w-[1500px] space-y-6 p-4 sm:p-6">
    <header className="rounded-3xl border border-cyan-900/30 bg-gradient-to-br from-slate-950 via-slate-900 to-cyan-950 p-6 text-white shadow-xl">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-black">Én identitet per brand. Riktig pipeline per business.</h1>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-300">Nexus kombinerer brand-registry, business-pipeline og live Marketing Readiness. Brand styrer identitet og kanalregler; business-modellen styrer kundereise, steg og konvertering.</p>
        </div>
    ...
    <section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5 text-emerald-950">
      <div className="flex items-start gap-3"><ShieldCheck size={21} className="mt-0.5 shrink-0" /><div><div className="font-black">Umbrella-brand er ikke en universell salgspipeline</div><p className="mt-1 text-sm leading-6 text-emerald-800">Freddy Bremseth-profesjonell kan skape interesse for bøker, AI, rådgivning eller eiendom. Offer-intent skal route henvendelsen til riktig business-pipeline. Privatprofilen er fortsatt ikke en automatisert kommersiell publiseringskanal.</p></div></div>
    </section>
    ...
    <section className="grid gap-5 xl:grid-cols-3">
      {states.map((state) => <article key={state.brand.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        {state.business && <div className="mt-4 rounded-xl border border-violet-200 bg-violet-50 p-4 text-violet-950">
          <div className="flex flex-wrap items-center justify-between gap-2"><div className="text-[10px] font-black uppercase tracking-wider text-violet-700">Business pipeline · {state.business.binding.role}</div><div className="rounded-full bg-white px-2 py-1 text-[10px] font-black text-violet-700">{state.business.pipeline.valueModel.replaceAll("_", " ")}</div></div>
          <div className="mt-2 font-black">{state.business.pipeline.name}</div>
          <div className="mt-1 text-xs leading-5 text-violet-800">Kundetype: {state.business.pipeline.customerLabel} · Suksess: {state.business.pipeline.successEvent}</div>
          <div className="mt-3 flex flex-wrap gap-1.5">{state.business.pipeline.stages.map((stage) => <span key={stage.id} className="rounded-md border border-violet-200 bg-white px-2 py-1 text-[10px] font-bold text-violet-800">{stage.label}</span>)}</div>
          {state.business.binding.role === "umbrella" && <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 p-2.5 text-xs leading-5 text-amber-900">Denne pipelinen er kun fallback. Bok-, AI-, eiendoms- og rådgivningssignaler skal routes til riktig offer-pipeline før Next Best Action beregnes.</div>}
        </div>}
