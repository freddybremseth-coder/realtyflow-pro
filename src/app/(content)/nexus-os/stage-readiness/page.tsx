...
export default function StageReadinessPage(){
  ...
  return <main className="mx-auto max-w-[1450px] space-y-6 p-4 sm:p-6">
    ...
    <section className="space-y-3"><div><h2 className="text-xl font-black text-slate-950">Arbeidskø</h2><p className="mt-1 text-sm text-slate-500">Høyeste pipeline-verdi først. Åpne Customer 360 og utfør bare det manglende steget.</p></div>{(data?.rows||[]).map(row=><article key={row.contactId} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div className="min-w-0 flex-1"><div className="flex flex-wrap gap-2 text-[10px] font-black uppercase tracking-wider text-slate-500"><span>{row.stage}</span><span>·</span><span>{row.brand||"ukjent brand"}</span><span>·</span><span>{LABELS[row.readiness]||row.readiness}</span></div><h3 className="mt-2 font-black text-slate-950">{row.name}</h3><div className="mt-1 text-xs text-slate-500">{row.email}</div><div className="mt-3 rounded-xl bg-cyan-50 p-3 text-sm text-cyan-950"><b>Neste steg:</b> {row.nextAction}{row.targetStage&&<div className="mt-1 text-xs font-black">Mål: {row.stage} → {row.targetStage}</div>}</div><div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-600"><span>Profil: {row.profile?.status||"ingen"}</span><span>· kriterier: {row.criteriaCount}</span><span>· shortlists: {row.shortlistCount}</span><span>· boliger: {row.shortlistItemCount}</span><span>· positive signaler: {row.interestedItemCount}</span></div></div><Link href={row.href} className="inline-flex items-center rounded-xl bg-slate-950 px-3 py-2 text-sm font-black text-white">Åpne Customer 360 <ArrowRight size={14} className="ml-2"/></Link></div></article>)}{!loading&&data&&data.rows.length===0&&<div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5 text-sm text-emerald-900"><CheckCircle2 size={16} className="mr-2 inline"/>Ingen QUALIFIED/MATCHING/VIEWING-kunder trenger stage readiness-arbeid.</div>}</section>
