...
type PriorityPayload = { items:Array<{bucket:string;activationHref:string;contact:{id:string}}> };
...
export default function ProfileActivationPage(){
  ...
  async function load(){
    ...
    setLoading(true);setError("");setNextHref(null);
    try{
  ...
  async function loadNextReadyCandidate(){
    try{
      const r=await fetch("/api/nexus/profile-activation-priority",{cache:"no-store",credentials:"same-origin"});
      const b=await r.json().catch(()=>null) as PriorityPayload|null;
      if(!r.ok||!b)return;
      const next=b.items.find(row=>row.bucket==="READY_TO_APPROVE"&&row.contact.id!==contactId);
      setNextHref(next?.activationHref||null);
    }catch{
      setNextHref(null);
    }
  }
  ...
  async function approve(){
    ...
    setSaving(true);setError("");setSuccess("");setNextHref(null);
    try{
  ...
  return <main className="mx-auto max-w-5xl space-y-6 p-4 sm:p-6">
    ...
    {success&&<div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm font-bold text-emerald-950"><CheckCircle2 size={16} className="mr-2 inline"/>{success}{nextHref?<Link href={nextHref} className="ml-3 inline-flex rounded-lg bg-emerald-800 px-3 py-1.5 text-xs font-black text-white">Neste kunde</Link>:<Link href="/nexus-os/profile-activation-priority" className="ml-3 inline-flex rounded-lg border border-emerald-300 bg-white px-3 py-1.5 text-xs font-black text-emerald-900">Til prioriteringskø</Link>}</div>}
