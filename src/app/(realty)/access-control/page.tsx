...
export default function AccessControlPage() {
  ...
  const [form, setForm] = useState({ email: "", displayName: "", role: "SALES", active: true });
  ...
  const reset = () => setForm({ email: "", displayName: "", role: "SALES", active: true });

  const save = async (overrideActive?: boolean) => {
    if (!form.email.trim()) return setError("E-post må fylles inn.");
    const active = overrideActive ?? form.active;
    if (!active && !confirm(`Deaktivere tilgang for ${form.email}? Eksisterende sesjon stoppes ved neste API-kall.`)) return;
    setSaving(true); setError(""); setNotice("");
    const response = await fetch("/api/access-control", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action: "UPSERT_PROFILE", ...form, active }),
    });
    const body = await response.json().catch(() => ({}));
    if (!response.ok) setError(body.error || "Kunne ikke lagre tilgangsprofilen.");
    else {
      setNotice(active ? "Tilgangsprofilen er lagret." : "Tilgangen er deaktivert.");
      reset();
      await load();
    }
    setSaving(false);
    ...
    <div className="min-h-screen bg-slate-950 p-4 text-slate-100 md:p-8">
      <div className="mx-auto max-w-7xl space-y-6">
        <header className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div>
            <div className="mb-2 flex items-center gap-2 text-sm font-medium text-emerald-400"><ShieldCheck size={18}/> Owner-only</div>
            <h1 className="text-3xl font-bold">Roller & tilgang</h1>
            <p className="mt-2 max-w-3xl text-slate-400">Tildel avgrensede roller til brukere som allerede finnes i Supabase Auth. RealtyFlow oppretter ikke konto, passord eller invitasjon.</p>
          </div>
        ...
        <section className="grid gap-6 lg:grid-cols-[1fr_1.4fr]">
          <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
            ...
            <div className="space-y-4">
              <label className="block text-sm text-slate-300">E-post
                <input value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} type="email" className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2" placeholder="bruker@firma.no"/>
              </label>
              <label className="block text-sm text-slate-300">Navn
                <input value={form.displayName} onChange={(e) => setForm({ ...form, displayName: e.target.value })} className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2" placeholder="Valgfritt visningsnavn"/>
              </label>
              <label className="block text-sm text-slate-300">Rolle
                <select value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })} className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2">
              ...
              <div className="flex gap-2">
                <button disabled={saving} onClick={() => void save()} className="flex-1 rounded-lg bg-primary-600 px-4 py-2 font-medium hover:bg-primary-500 disabled:opacity-50">{saving ? "Lagrer…" : "Lagre profil"}</button>
                <button onClick={reset} className="rounded-lg border border-slate-700 px-4 py-2 hover:bg-slate-800">Nullstill</button>
              </div>
            </div>
          </div>
              ...
              {(data?.settings.profiles || []).map((profile) => (
                <div key={profile.email} className={`rounded-xl border p-4 ${profile.active ? "border-slate-700 bg-slate-950/50" : "border-slate-800 bg-slate-950/20 opacity-65"}`}>
                  <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    ...
                    <div className="flex gap-2"><button onClick={() => edit(profile)} className="rounded-lg border border-slate-700 px-3 py-2 text-sm hover:bg-slate-800">Rediger</button>{profile.active && <button onClick={() => { setForm({ email: profile.email, displayName: profile.displayName || "", role: profile.role, active: false }); setTimeout(() => void save(false), 0); }} className="rounded-lg border border-red-800 px-3 py-2 text-sm text-red-300 hover:bg-red-950/40">Deaktiver</button>}</div>
                  </div>
                </div>
              ))}
        ...
        <section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
          <h2 className="mb-4 text-lg font-semibold">Rollematrise</h2>
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {(data?.roles || []).map((role) => <div key={role.id} className="rounded-xl border border-slate-800 bg-slate-950/50 p-4"><div className="font-semibold">{role.label}</div><div className="mt-1 text-xs text-slate-500">{role.id}</div><div className="mt-3 flex flex-wrap gap-1.5">{role.permissions.map((permission) => <span key={permission} className="rounded bg-slate-800 px-2 py-1 text-[11px] text-slate-300">{permission}</span>)}</div></div>)}
          </div>
        </section>
      </div>
    </div>
