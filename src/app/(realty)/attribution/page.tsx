...
type CanonicalRow = { brandId: string; channel: string; campaignId: string; leads: number; qualified: number; viewings: number; offers: number; sales: number; commissionEur: number; assistedConversions: number };
type CanonicalReport = { sourceOfTruth: string; deterministicOnly: true; model: string; summary: { leads: number; qualified: number; viewings: number; offers: number; sales: number; commissionEur: number; assistedConversions: number; attributedLeads: number; coveragePercent: number }; brands: CanonicalRow[]; channels: CanonicalRow[]; campaigns: CanonicalRow[] };
type ResponseData = { workspace: Workspace; canonical: CanonicalReport; config: { spend: Array<{ sourceId: string; spendEur: number }>; notes: string; updatedAt: string | null }; sourceOptions: Array<{ id: string; label: string }> };
...
export default function AttributionPage() {
  ...
  return <div className="mx-auto max-w-7xl space-y-6">
    ...
    {loading && !data ? <div className="flex min-h-52 items-center justify-center rounded-xl border border-slate-700 bg-slate-900/50 text-slate-400"><Loader2 size={20} className="mr-2 animate-spin" />Bygger attribusjon …</div> : data ? <>
      ...
      <section className="rounded-2xl border border-emerald-500/25 bg-slate-900/60 p-5">
        <div className="mb-4 flex flex-col gap-2 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-xs uppercase tracking-wide text-emerald-300">Measurement & Revenue Loop</p><h2 className="text-xl font-semibold text-white">Dokumentert lead-attribusjon</h2><p className="mt-1 text-sm text-slate-400">Kilden er {data.canonical.sourceOfTruth}. Bare deterministiske touchpoints brukes; ukjente felt forblir ukjent.</p></div><span className="w-fit rounded-full border border-emerald-500/30 bg-emerald-500/10 px-3 py-1 text-xs text-emerald-200">{data.canonical.model}</span></div>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><Metric label="Canonical leads" value={data.canonical.summary.leads} icon={Users} /><Metric label="Attribusjonsdekning" value={percent(data.canonical.summary.coveragePercent)} icon={ShieldCheck} /><Metric label="Salg" value={data.canonical.summary.sales} icon={CheckCircle2} /><Metric label="Provisjon" value={money(data.canonical.summary.commissionEur)} icon={CircleDollarSign} /><Metric label="Assisterte salg" value={data.canonical.summary.assistedConversions} icon={Target} /></div>
        <div className="mt-5 overflow-x-auto rounded-xl border border-slate-700/70"><table className="w-full min-w-[760px] text-left text-sm"><thead className="bg-slate-950 text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Brand / kanal</th><th className="px-4 py-3">Leads</th><th className="px-4 py-3">Kvalifisert</th><th className="px-4 py-3">Visninger</th><th className="px-4 py-3">Tilbud</th><th className="px-4 py-3">Salg</th><th className="px-4 py-3">Provisjon</th></tr></thead><tbody>{data.canonical.channels.length ? data.canonical.channels.map((row) => <tr key={`${row.brandId}-${row.channel}`} className="border-t border-slate-800 bg-slate-950/40"><td className="px-4 py-3 font-medium text-white">{row.brandId} · {row.channel}</td><td className="px-4 py-3 text-slate-300">{row.leads}</td><td className="px-4 py-3 text-slate-300">{row.qualified}</td><td className="px-4 py-3 text-slate-300">{row.viewings}</td><td className="px-4 py-3 text-slate-300">{row.offers}</td><td className="px-4 py-3 text-slate-300">{row.sales}</td><td className="px-4 py-3 text-slate-300">{money(row.commissionEur)}</td></tr>) : <tr><td colSpan={7} className="px-4 py-8 text-center text-slate-500">Ingen canonical lead-reiser i valgt kohort.</td></tr>}</tbody></table></div>
        {data.canonical.campaigns.length > 0 && <div className="mt-5"><h3 className="mb-3 text-sm font-semibold text-white">Kampanjer</h3><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{data.canonical.campaigns.map((row) => <article key={`${row.brandId}-${row.channel}-${row.campaignId}`} className="rounded-lg border border-slate-700 bg-slate-950/50 p-4"><p className="text-xs text-slate-500">{row.brandId} · {row.channel}</p><strong className="mt-1 block text-white">{row.campaignId}</strong><p className="mt-2 text-xs text-slate-400">{row.leads} leads · {row.sales} salg · {money(row.commissionEur)}</p></article>)}</div></div>}
      </section>
