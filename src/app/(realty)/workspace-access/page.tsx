...
export default function WorkspaceAccessPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 text-slate-100">
      ...
      {payload?.zenJointPreview && brandKey === "zeneco" && <ZenJointLeadReview />}
      {payload && (
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1.3fr)_minmax(300px,1fr)]">
          <section className="space-y-5 rounded-2xl border border-slate-800 bg-slate-900/80 p-5">
            ...
            <label className="block text-sm text-slate-300">Merkevare
              <select className={input} value={brandKey} onChange={(event) => {
                const nextBrand = event.target.value;
                setBrandKey(nextBrand);
                setPermissions(nextBrand === "zeneco"
                  ? [...suggested.filter(permission => permission !== "crm.read" && permission !== "crm.write"), "crm.joint.read", "crm.joint.write", "tasks.joint.read", "tasks.joint.write"]
                  : suggested);
              }}>
            ...
            {brandKey === "zeneco" && !payload.zenJointPreview && <p role="status"
              className="rounded-xl border border-amber-700/60 bg-amber-950/25 p-4 text-sm text-amber-200">
              Oversikten over nye CRM-poster er ikke installert ennå. Det er ikke registrert en nullverdi:
              antall kandidater og godkjente felleskunder er foreløpig ukjent. Eksisterende tilgangsutkast
              kan fremdeles gjennomgås; ingen kunde- eller medarbeidertilgang aktiveres her.
            </p>}
