...
export async function GET(request: NextRequest) {
  ...
  const [organizationResult, settingsResult, customersResult, productsResult, taxRulesResult, documentsResult, paymentsResult, refundsResult, seriesResult, jobsResult] = await Promise.all([
    ...
    supabase.from("billing_refunds").select("*").eq("organization_id", organizationId).order("refund_date", { ascending: false }).limit(100),
  ...
  const failed = [organizationResult, settingsResult, customersResult, productsResult, taxRulesResult, documentsResult, paymentsResult, refundsResult, seriesResult, jobsResult].find((result) => result.error);
