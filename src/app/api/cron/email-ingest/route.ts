...
export async function GET(request: NextRequest) {
  ...
  const now = Date.now();
  const results: Array<{ brand: string; email: string; fetched: number; inserted: number; skipped?: string; error?: string }> = [];
  let totalInserted = 0;
  let totalFetched = 0;
  ...
  for (const config of configs ?? []) {
    ...
    if (config.last_fetched_at && now - new Date(config.last_fetched_at).getTime() < intervalMs) {
      results.push({ brand: config.brand_id, email: config.email_address, fetched: 0, inserted: 0, skipped: "not_due" });
      continue;
    }
    ...
    try {
      ...
      const sinceDays = config.last_fetched_at
        ? Math.max(1, Math.min(30, Math.ceil((now - new Date(config.last_fetched_at).getTime()) / 86_400_000)))
        : 7;
      ...
      await supabase.from("brand_email_configs").update({ last_fetched_at: fetchedAt }).eq("id", config.id);
      results.push({ brand: config.brand_id, email: config.email_address, fetched: fetched.length, inserted: insertedCount });
    } catch (e) {
      results.push({ brand: config.brand_id, email: config.email_address, fetched: 0, inserted: 0, error: e instanceof Error ? e.message : String(e) });
    }
  }
