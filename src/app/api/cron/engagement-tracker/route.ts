...
async function getCanonicalToken(brand: string, platform: "facebook" | "instagram") {
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log("[Engagement Tracker] Starting with canonical social_channels only...");
    ...
    for (const post of posts) {
      if (post.facebook_post_id) {
        const token = await getCanonicalToken(post.brand_id, "facebook");
        if (token) {
          ...
          console.warn(`[Engagement Tracker] No unique canonical Facebook channel for ${post.brand_id}; skipped.`);
        }
      }

      if (post.instagram_post_id) {
        const token = await getCanonicalToken(post.brand_id, "instagram");
        if (token) {
          try {
            ...
            await supabase.from("engagement_snapshots").insert({
              ...
              raw_data: { ...engagement.raw, views: engagement.views, saves: engagement.saves, total_interactions: engagement.totalInteractions },
            });
            ...
            if (post.source_social_post_id) {
              ...
              if (socialPost) {
                await supabase.from("social_post_metrics").insert({
                  ...
                  shares: engagement.shares,
                  saves: engagement.saves,
                  notes: "Automatisk importert fra Instagram Insights via canonical social_channels",
                });
              }
            }
            tracked++;
          ...
          console.warn(`[Engagement Tracker] No unique canonical Instagram channel for ${post.brand_id}; skipped.`);
        }
      }
    }
    ...
    if (tracked > 0) {
      ...
      if (snapshots?.length) {
        ...
        const { data: pubs } = await supabase.from("content_publications").select("id, brand_id, published_at").in("id", pubIds);
        ...
        for (const agg of Array.from(aggregated.values())) {
          await supabase.from("scheduling_insights").upsert({
            brand_id: agg.brand,
    ...
    return NextResponse.json({ message: `Tracked ${tracked} posts`, tracked, channelSource: "social_channels" });
  } catch (err) {
    console.error("[Engagement Tracker] Error:", err);
    return NextResponse.json({ error: err instanceof Error ? err.message : "Tracking feilet" }, { status: 500 });
  }
}
