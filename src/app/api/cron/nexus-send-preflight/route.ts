...
export async function GET(request: NextRequest) {
  ...
  const work = await supabase
    .from("work_items")
    .select("id,brand_id,status,next_action,metadata,updated_at")
    .eq("source_type", "crm")
    .in("status", OPEN_STATUSES)
    .eq("metadata->>presentation_send_preflight_required", "true")
  ...
  for (const row of work.data || []) {
    ...
    try {
      ...
      const nextMetadata = {
        ...metadata,
        send_preflight_status: assessment.status,
        send_preflight_ready: assessment.ready,
        send_preflight_checked_at: now,
        send_preflight_checked_by: ACTOR,
        send_preflight_blockers: assessment.blockers.slice(0, 20),
        send_preflight_warnings: assessment.warnings.slice(0, 30),
        send_preflight_checks: assessment.checks,
        send_preflight_revalidate_at_send: true,
        presentation_customer_send_allowed: false,
        presentation_send_preflight_required: assessment.ready ? false : true,
      };
      const nextAction = assessment.ready
        ? "Send-preflight er grønn. Ingen melding er sendt. Neste steg krever eksplisitt send-godkjenning, og alle sikkerhetssjekker skal kjøres på nytt rett før utsending."
        : `Send-preflight er blokkert: ${assessment.blockers.slice(0, 3).join(" ")}`;

      const update = await supabase.from("work_items")
      ...
      failed += 1;
      console.warn("[nexus-send-preflight] work item failed", {
        workItemId: row.id,
  ...
  await supabase.from("automation_logs").insert({
    action: "nexus_send_preflight",
    agent_name: "nexus_send_preflight",
    status: failed ? (ready || blocked ? "partial" : "failed") : "success",
    details: {
      considered,
      ready,
      blocked,
