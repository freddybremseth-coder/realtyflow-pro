...
export async function GET(request: NextRequest) {
  ...
  const { data: runtime, error: runtimeError } = await supabase
    ...
    .maybeSingle();
  if (runtimeError) return NextResponse.json({ error: runtimeError.message }, { status: 500 });
  if (!runtime?.enabled) return NextResponse.json({ success: true, skipped: true, reason: "feature:social_inbox_sync is disabled" });
  ...
  await supabase.from("automation_logs").insert({
    ...
    agent_name: "nexus_social_inbox_sync_cron",
    status: results.some((r) => r.error) ? "partial" : "success",
    details: { comments_fetched: commentsFetched, conversations_upserted: conversationsUpserted, messages_upserted: messagesUpserted, runtime_control: "feature:social_inbox_sync", read_only: true },
  }).catch(() => undefined);

  return NextResponse.json({ success: true, readOnly: true, commentsFetched, conversationsUpserted, messagesUpserted, results });
}
