...
import { AlertTriangle, ArrowLeft, Check, CircleDollarSign, Copy, CreditCard, Eye, FileCheck2, Loader2, Mail, Plus, Save, Send, Trash2 } from "lucide-react";
...
import type { BillingCustomer, BillingDocument, BillingDocumentLine, BillingDocumentType, BillingLineInput, BillingOrganization, BillingProduct, BillingTaxRule } from "@/lib/billing/types";
...
type Bundle = { document: BillingDocument; customer: BillingCustomer; organization: BillingOrganization; lines: BillingDocumentLine[]; snapshot: Record<string, any> | null };
...
const STATUS_LABELS: Record<string, string> = { draft: "Kladd", ready: "Klar", issued: "Utstedt", sent: "Sendt", opened: "Åpnet", partially_paid: "Delvis betalt", paid: "Betalt", overdue: "Forfalt", credited: "Kreditert", replaced: "Erstattet" };
...
export function BillingDocumentEditor({ initialDocumentId = null }: { initialDocumentId?: string | null }) {
  ...
  const runAction = async (body: Record<string, unknown>, success: string) => {
    let id = documentId; if (!locked && body.action !== "payment" && body.action !== "send") id = await persistDraft(); if (!id) return null;
  ...
  const reloadBundle = async (id = documentId) => { if (!id) return; const response = await fetch(`/api/billing/documents/${id}`, { cache: "no-store" }); if (response.ok) { const next = await response.json(); setBundle(next); setPayment((current) => ({ ...current, amount: String(next.document.balance) })); } };
