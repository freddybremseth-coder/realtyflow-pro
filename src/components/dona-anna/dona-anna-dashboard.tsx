...
const selectClass = "h-10 w-full rounded-xl border border-white/10 bg-white/[0.03] px-3 text-sm text-slate-100 outline-none focus:border-green-500";
const textareaClass = "min-h-24 w-full rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2 text-sm text-slate-100 outline-none focus:border-green-500";
...
export function DonaAnnaDashboard() {
  ...
  return <div className="olivia-app mx-auto max-w-[1600px] space-y-5 bg-[#0a0a0b]" style={{ fontFamily: "Roboto, sans-serif" }}>
    <header className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
      <div>
        <div className="flex flex-wrap items-center gap-2">
          <div><h1 className="font-['Space_Mono'] text-3xl font-bold text-white">Olivia <span className="text-green-400">OS</span></h1><p className="text-[10px] uppercase tracking-[0.28em] text-slate-500">Doña Anna</p></div>
    ...
    {message && <div role="status" className="rounded-xl border border-green-500/30 bg-green-500/10 p-4 text-sm text-green-100"><CheckCircle2 className="mr-2 inline" size={16} />{message}</div>}
    ...
    <nav aria-label="Doña Anna-moduler" className="flex gap-1 overflow-x-auto rounded-xl border border-slate-700/60 bg-slate-900/60 p-1.5">
      {tabs.map(([id, label, Icon]) => <button type="button" key={id} aria-current={tab === id ? "page" : undefined} onClick={() => setTab(id)} className={`flex shrink-0 items-center gap-2 rounded-lg px-3 py-2 text-sm ${tab === id ? "bg-green-500 text-black shadow-[0_0_20px_rgba(34,197,94,0.3)]" : "text-slate-400 hover:bg-slate-800 hover:text-white"}`}><Icon size={15} />{label}</button>)}
    </nav>
...
function Overview({ data }: { data: DonaAnnaSnapshot }) {
  const cards = [
    ["Produkter", data.metrics.productCount, Package, "text-green-400"],
    ["Varehus", data.metrics.warehouseCount, Warehouse, "text-blue-400"],
    ["Batcher", data.metrics.lotCount, ScanLine, "text-violet-300"],
    ["På lager", quantity(data.metrics.onHand), Boxes, "text-yellow-500"],
...
function Readiness({ ok, label, detail }: { ok: boolean; label: string; detail: string }) {
  return <div className="flex items-start gap-3 rounded-lg border border-slate-700/60 bg-slate-900/40 p-4">{ok ? <CheckCircle2 className="mt-0.5 text-green-400" size={18} /> : <AlertTriangle className="mt-0.5 text-yellow-500" size={18} />}<div><p className="font-medium text-white">{label}</p><p className="mt-1 text-xs text-slate-500">{detail}</p></div></div>;
}
...
function Products({ data, busy, runCommand }: { data: DonaAnnaSnapshot; busy: string; runCommand: Runner }) {
  ...
  return <div className="grid gap-5 xl:grid-cols-[420px_minmax(0,1fr)]">
    ...
    <Card><CardHeader><CardTitle>Produktkatalog</CardTitle><CardDescription>{data.products.length} aktive produkter.</CardDescription></CardHeader><CardContent><div className="grid gap-3 md:grid-cols-2 2xl:grid-cols-3">{data.products.map((product) => <div key={product.id} className="rounded-3xl border border-white/10 bg-white/[0.03] backdrop-blur-md hover:border-white/20 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-medium text-white">{product.name}</p><p className="font-mono text-xs text-slate-500">{product.sku} · {product.unit}</p></div><Badge variant={product.track_lots ? "default" : "secondary"}>{product.track_lots ? "Batch" : "Uten batch"}</Badge></div><p className="mt-3 line-clamp-2 text-sm text-slate-400">{product.description || "Ingen beskrivelse"}</p><p className="mt-4 font-semibold text-green-400">{money(product.unit_price, product.price_currency || "EUR")}</p></div>)}{data.products.length === 0 && <Empty text="Ingen produkter ennå." />}</div></CardContent></Card>
  </div>;
}
