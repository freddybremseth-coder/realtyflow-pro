...
/** One Freddy publishing/identity family, but exact-host search numbers per site. */
const FREDDY_FAMILY = [
  { brandId: "freddyb", url: "https://www.freddybremseth.com", title: "Hovednettstedet",
    purpose: "Din profil, eiendomsrådgivning og inngang til bøkene og kunsten." },
  { brandId: "freddypublishing", url: "https://books.freddybremseth.com", title: "Bøker",
          ...
          <section aria-label="Freddy Bremseth nettstedfamilie" className="mt-4 rounded-xl border border-slate-300 bg-white p-4 text-slate-950">
            ...
            <div className="mt-3 grid gap-3 lg:grid-cols-3">
              {FREDDY_FAMILY.map(site => {
                ...
                return (
                  <article key={site.brandId} className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm">
                    <a href={site.url} target="_blank" rel="noopener noreferrer"
                      className="font-black text-slate-950 underline underline-offset-2">{site.title} · {new URL(site.url).hostname}</a>
                    <p className="mt-2 text-slate-700">{site.purpose}</p>
                    {metric ? (
                      <p className="mt-2 font-semibold text-emerald-950">
                        Google web-søk ({metric.period.currentStart}–{metric.period.currentEnd}):
                        {" "}{metric.totals.currentClicks} klikk · {metric.totals.currentImpressions} visninger.
                      </p>
                    ) : (
                      <p className="mt-2 font-semibold text-amber-900">
                        Egne Google-søketall er ikke verifisert for denne adressen.
                      </p>
                    )}
