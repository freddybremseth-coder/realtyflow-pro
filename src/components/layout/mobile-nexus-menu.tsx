...
export function MobileNexusMenu() {
  ...
  useEffect(() => {
    ...
    fetch("/api/auth/me", { cache: "no-store" })
      ...
      .then((body) => {
        if (alive && body?.user) setUser(body.user as CurrentUser);
      })
      .catch(() => undefined);
    return () => { alive = false; };
  }, [router]);
  ...
  const filtered = useMemo(() => {
    ...
    return sections
      .map((section) => ({
        ...section,
        items: section.items.filter((item) => !q || `${item.label} ${item.href}`.toLowerCase().includes(q)),
      }))
  ...
  return <>
    <header className="fixed inset-x-0 top-0 z-40 flex h-14 items-center justify-between border-b border-slate-700 bg-slate-950 px-3 shadow-lg lg:hidden">
      <Link href="/nexus-os/today" className="flex items-center gap-2">
        <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-cyan-300 text-sm font-black text-slate-950">RF</div>
        <div className="leading-tight">
          <div className="text-sm font-black text-white">RealtyFlow Pro</div>
          <div className="text-[9px] font-bold uppercase tracking-[.18em] text-cyan-200">Arbeidsmeny</div>
        </div>
      </Link>
      <button onClick={() => setOpen(true)} aria-label="Åpne meny" className="flex h-9 w-9 items-center justify-center rounded-xl border border-slate-600 bg-slate-900 text-white"><Menu size={19} /></button>
    </header>
    ...
    {!open ? null : <div className="fixed inset-0 z-[80] bg-slate-950 text-white lg:hidden">
      <div className="flex h-14 items-center justify-between border-b border-slate-700 px-4">
        <div>
          <div className="text-sm font-black text-white">Hvor vil du jobbe?</div>
          <div className="text-[10px] font-medium text-slate-300">Daglige valg først · resten under arbeidsområder</div>
        </div>
      ...
      <div className="h-[calc(100dvh-3.5rem)] overflow-y-auto pb-24">
        ...
        <div className="sticky top-0 z-10 bg-slate-950 p-4">
          <label className="relative block">
            ...
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Søk etter kunder, e-post, eiendom …" className="w-full rounded-xl border border-slate-600 bg-slate-900 py-2.5 pl-10 pr-3 text-sm text-white placeholder:text-slate-400 outline-none focus:border-cyan-400 focus:ring-1 focus:ring-cyan-400" />
          </label>
        </div>
