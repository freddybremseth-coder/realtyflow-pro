...
test("known API routes map to explicit permissions and unknown routes stay owner-only", () => {
  ...
  assert.equal(accessRequirementForApi("/api/workspaces/zeneco/joint-contacts", "PATCH"), "OWNER_ONLY");
...
test("navigation is reduced by role", () => {
  ...
  assert.equal(canSeeNavHref("OWNER", "/platform"), true);
  assert.equal(canSeeNavHref("FINANCE", "/platform"), false);
