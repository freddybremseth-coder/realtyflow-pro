...
export function isCorporateHomeLead(contact: Record<string, any>) {
  const haystack = [contact.source, contact.notes, contact.property_interest, interactionText(contact.interactions)]
    ...
    .join("\n")
    .toLowerCase();
  return haystack.includes("zeneco-corporate-homes") ||
    haystack.includes("corporate-home") ||
    haystack.includes("bedriftshytte") ||
    haystack.includes("medlemsbolig") ||
    haystack.includes("shared corporate home");
}
...
function nextActionFor(
  status: unknown,
  profile: { users: number | null; role: string | null; budgetLabel: string | null; timeline: string | null },
) {
  const stage = text(status).toUpperCase();
  if (stage === "NEW") {
...
export function corporateLeadProfile(contact: Record<string, any>): CorporateLeadProfile {
  ...
  return {
    ...
    score,
    priority,
    stageLabel: stageLabel(contact.pipeline_status),
    nextAction: nextActionFor(contact.pipeline_status, { users, role, budgetLabel, timeline }),
  };
}
