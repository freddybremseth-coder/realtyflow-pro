...
/**
 * Deterministic first-pass classifier for inbound customer email/replies.
 * It intentionally favors explicit customer language and conservative
 * escalation. AI enrichment may add evidence later, but must not override an
 * explicit do-not-contact signal.
 */
export function classifyInboundReply(input: { subject?: string | null; body?: string | null }): InboundReplyClassification {
  ...
  if (dnc) {
    return result("do_not_contact", 0.995, "suppress_contact", ["Explicit do-not-contact signal detected."], {
      shouldStopNurture: true,
    });
  }
  ...
  if (purchasedElsewhere) {
    return result("purchased_elsewhere", 0.98, "mark_lost_purchased_elsewhere", ["Customer states that a property has already been purchased."], {
      shouldStopNurture: true,
    });
  }
  ...
  if (viewing) {
    return result("viewing_request", 0.97, "prioritize_viewing", ["Explicit viewing intent detected."], {
      shouldPauseNurture: true,
      requiresFastResponse: true,
      shouldRunPropertyMatching: true,
    });
  }

  const specificProperty = /\b(this property|that property|the apartment|the villa|the house|denne boligen|den boligen|denne leiligheten|denne villaen|ref\.?\s*[a-z0-9-]+|reference\s*[a-z0-9-]+)\b/i.test(text)
    || /https?:\/\//i.test(text);
  if (specificProperty) {
    return result("property_interest", 0.93, "prioritize_property_match", ["Customer references a specific property or property link."], {
      shouldPauseNurture: true,
      shouldRunPropertyMatching: true,
      requiresFastResponse: true,
    });
  ...
  if (changed) {
    return result("update_preferences", 0.91, "refresh_buyer_profile", ["Customer indicates changed buying requirements."], {
      shouldPauseNurture: true,
  ...
  if (later) {
    return result("follow_up_later", 0.9, "schedule_followup", ["Customer asks for a later follow-up."], {
      shouldPauseNurture: true,
    });
  }
  ...
  if (active) {
    return result("active_interest", 0.91, "move_to_contact", ["Customer confirms active buying interest."], {
      shouldPauseNurture: true,
      shouldRunPropertyMatching: true,
      requiresFastResponse: true,
    });
  ...
  if (question) {
    return result("question", 0.82, "prepare_answer", ["Inbound message contains a customer question."], {
      shouldPauseNurture: true,
      requiresFastResponse: true,
    });
  }

  return result("unclear", 0.35, "manual_review", ["No sufficiently clear commercial intent detected."], {
    shouldPauseNurture: true,
  });
}

/**
 * Connects inbound intent to the central AUTO / REVIEW / FREDDY policy.
 * Terminal commercial outcomes stay human-controlled in v1, while explicit
 * suppression may be applied automatically because continuing outbound contact
 * would be the unsafe action.
 */
export function governInboundReply(classification: InboundReplyClassification): GovernedInboundReplyDecision {
  if (classification.intent === "do_not_contact") {
    const safety: AutopilotSafetyDecision = {
      tier: "AUTO",
      allowed: true,
      reason: "Explicit do-not-contact request should be honored immediately",
      requiresAudit: true,
    };
  ...
  const safety = decideAutopilotTier({
    actionType: classification.shouldRefreshBuyerProfile ? "buyer_profile_activation" : "data_update",
    risk,
    confidence: classification.confidence,
    requiredDataComplete: classification.intent !== "unclear",
  });
  return { classification, safety, canApplyAutomatically: safety.tier === "AUTO" && safety.allowed };
}
