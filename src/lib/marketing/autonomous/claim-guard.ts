...
const OUTCOME_CLAIM_MARKERS: Array<{ label: string; re: RegExp }> = [
  ...
  { label: "energy-efficient property", re: /(?:energieffektiv(?:e|t)?\s+(?:bolig(?:en|er|ene)?|villa(?:en|er|ene)?|leilighet(?:en|er|ene)?|hjem(?:met)?|boligprosjekt(?:et|er|ene)?)|energy[-\s]?efficient\s+(?:home|villa|apartment|property|residence|development))/i },
