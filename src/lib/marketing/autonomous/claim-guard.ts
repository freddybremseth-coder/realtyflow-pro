...
const INVENTORY_NARRATIVE_MARKERS: Array<{ label: string; re: RegExp }> = [
  ...
  { label: "location praise", re: /\b(?:i|in)\s+(?:vakr(?:e|t)?|nydelig(?:e|t)?|beautiful|stunning)\s+[a-zæøåáéíóúüñ-]+(?:\s+[a-zæøåáéíóúüñ-]+){0,4}/i },
  { label: "excellent opportunity", re: /\b(?:utmerket|svært\s+god|fantastisk|excellent|great|fantastic)\s+(?:mulighet|opportunity)\b/i },
  { label: "subjective property praise", re: /(?:flott(?:e|t)?|fin(?:e|t)?|great|lovely|nice)\s+(?:rekkehus(?:et)?|townhouse|bolig(?:en|er|ene)?|villa(?:en|er|ene)?|eiendom(?:men|mer|mene)?|leilighet(?:en|er|ene)?|home|property|apartment|residence)/i },
  { label: "modern amenities", re: /(?:moderne|modern)\s+(?:fasiliteter|fasilitetene|amenities|facilities)/i },
  { label: "new-home framing", re: /(?:oppdag|finn|discover|find)\s+(?:ditt|deres|your)\s+(?:nye?\s+)?(?:hjem|home)/i },
  { label: "home-in-the-sun suitability", re: /(?:ønsker|ser\s+etter|drømmer\s+om|wants?|looking\s+for|dreams?\s+of)[^.!?]{0,70}(?:hjem|bolig|home|property)[^.!?]{0,30}(?:i\s+solen|in\s+the\s+sun)/i },
  ...
  { label: "holiday-and-permanent suitability", re: /(?:perfekt|ideell|egnet|passer|perfect|ideal|suitable)[^.!?]{0,55}(?:ferie|holiday)[^.!?]{0,55}(?:permanent\s+opphold|fast\s+bosetting|permanent\s+residence|permanent\s+living|year[-\s]?round\s+living)/i },
  { label: "permanent-home suitability", re: /(?:permanent\s+hjem|helårsbolig|permanent\s+home|year[-\s]?round\s+home)/i },
  { label: "spacious property", re: /(?:romslig(?:e|t)?|spacious)\s+(?:bolig(?:en|er|ene)?|villa(?:en|er|ene)?|eiendom(?:men|mer|mene)?|leilighet(?:en|er|ene)?|hjem(?:met)?|home|property|villa|apartment|residence)/i },
  { label: "perfect placement", re: /(?:perfekt\s+plassert|perfectly\s+(?:placed|located))\b/i },
  { label: "modern solution", re: /(?:moderne|modern)\s+(?:løsning|solution)\b/i },
  { label: "new-or-holiday-home suitability", re: /(?:ditt|deres|your)\s+(?:nye?\s+)?(?:hjem|home)[^.!?]{0,35}(?:eller|or)\s+(?:feriebolig|holiday\s+home)/i },
  { label: "life-in-the-sun lifestyle", re: /(?:liv(?:et)?\s+i\s+solen|life\s+in\s+the\s+sun)/i },
  { label: "safe advisory promise", re: /(?:trygg|sikker|safe|secure)\s+(?:norsk\s+)?(?:rådgivning|advice|guidance)/i },
