...
import {
  ...
  getNexusActionPolicy,
  listNexusActionPolicies,
  policyForRevenueAction,
} from "./action-policy-registry";
...
test("only explicitly safe actions may execute automatically", () => {
  ...
  for (const policy of listNexusActionPolicies().filter((item) => !safe.includes(item.actionType as (typeof safe)[number]))) {
    assert.equal(canExecuteAutomatically(policy), false);
  }
});
