...
const POLICIES: Record<NexusActionType, NexusActionPolicy> = {
  ...
  property_match_prepare: { actionType: "property_match_prepare", policyClass: "AUTO_SAFE", reversible: true, customerFacing: false, sideEffect: true, requiresFreshSafetyCheck: false, reason: "Internal property matching prepares candidates but does not send or approve anything for the customer." },
  ...
  commission_decision: { actionType: "commission_decision", policyClass: "HUMAN_REQUIRED", reversible: false, customerFacing: false, sideEffect: true, requiresFreshSafetyCheck: true, reason: "Commission actions affect financial claims and require human judgment." },
  commercial_approval: { actionType: "commercial_approval", policyClass: "HUMAN_REQUIRED", reversible: false, customerFacing: false, sideEffect: true, requiresFreshSafetyCheck: true, reason: "Commercial approvals remain a human decision boundary." },
  legal_or_contract_commitment: { actionType: "legal_or_contract_commitment", policyClass: "FORBIDDEN", reversible: false, customerFacing: true, sideEffect: true, requiresFreshSafetyCheck: true, reason: "Nexus must not autonomously enter, alter or promise legally binding contractual commitments." },
  price_or_availability_guarantee: { actionType: "price_or_availability_guarantee", policyClass: "FORBIDDEN", reversible: false, customerFacing: true, sideEffect: true, requiresFreshSafetyCheck: true, reason: "Nexus must not guarantee price, availability, scarcity or other facts that require fresh external verification." },
  wait: { actionType: "wait", policyClass: "WAIT", reversible: true, customerFacing: false, sideEffect: false, requiresFreshSafetyCheck: false, reason: "The correct action is to wait until a meaningful new signal or scheduled follow-up point exists." },
};
...
export function policyForRevenueAction(action: Pick<CommandAction, "source">): NexusActionPolicy {
  ...
  if (action.source === "today" || action.source === "recovery" || action.source === "service-revenue" || action.source === "after-sales") return POLICIES.general_customer_message;
  return POLICIES.wait;
}
export function canExecuteAutomatically(policy: NexusActionPolicy): boolean { return policy.policyClass === "AUTO_SAFE"; }
