...
test("Revenue Brain ranks commercial urgency and delegates policy to the registry", () => {
  const brain = buildRevenueBrain(command([
    { id: "followup-1", source: "today", priority: "HIGH", score: 82, title: "Prioritert salgsoppfølging", subject: "Kari", description: "Følg opp kunden.", value: 0, href: "/customers/kari", contactId: "kari" },
    { id: "closing-1", source: "closing", priority: "CRITICAL", score: 112, title: "Closing med høy risiko", subject: "Ola", description: "Avklar kontrakten.", value: 18000, href: "/closing", contactId: "ola" },
  ]));
...
test("Revenue Brain deduplicates multiple opportunities for the same contact", () => {
  const brain = buildRevenueBrain(command([
    { id: "today-same", source: "today", priority: "HIGH", score: 80, title: "Follow-up", subject: "Same customer", description: "Follow up.", value: 0, href: "/customers/1", contactId: "contact-1" },
    { id: "closing-same", source: "closing", priority: "CRITICAL", score: 108, title: "Closing", subject: "Same customer", description: "Protect the deal.", value: 12000, href: "/closing", contactId: "contact-1" },
  ]));
...
test("Revenue Brain limits output and includes explainable policy rationale", () => {
  const actions = Array.from({ length: 15 }, (_, index) => ({ id: `recovery-${index}`, source: "recovery" as const, priority: "HIGH" as const, score: 80 - index, title: "Recovery", subject: `Customer ${index}`, description: "Prepare recovery follow-up.", value: 1000 + index * 100, href: "/recovery", contactId: `contact-${index}` }));
