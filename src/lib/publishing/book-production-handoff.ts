...
export function handoffReadiness(project: Record<string, any>, hasCover: boolean) {
  ...
  if (project.status !== "ready_for_export") blocking.push("Project must be ready_for_export before production handoff.");
  ...
  warnings.push("Print interior PDF is not generated by the current Book Engine handoff.");
  warnings.push("KDP full-wrap cover is not generated until final print pagination is known.");
  return { ok: blocking.length === 0, blocking, warnings, productionStatus: blocking.length ? "incomplete" : "digital_ready" };
}
