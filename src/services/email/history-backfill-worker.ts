...
async function loadExistingMessageIds(supabase: SupabaseClient, brandId: string) {
  ...
  // Messages previously imported through a provider API may not yet carry the
  // RFC Message-ID in email_messages.message_id. Persisted IMAP aliases make
  // those rows skippable on later backfill runs as well.
