...
test("reconciles explicit terminal latest reply while ignoring quoted outbound text", () => {
  ...
  assert.match(candidates[0]?.latestReply || "", /Ikke aktuelt lenger/i);
  assert.doesNotMatch(candidates[0]?.latestReply || "", /fortsatt interessert/i);
});

test("quoted STOPP in an older thread does not suppress a later active reply", () => {
  const candidates = planInboundReplyReconciliation([
    row({
      id: "new-active-quoted-stop",
      body_text: "Ja, vi ser fortsatt etter bolig.\n\nOn Monday, customer@example.com wrote:\nStopp. Ikke kontakt meg videre.",
