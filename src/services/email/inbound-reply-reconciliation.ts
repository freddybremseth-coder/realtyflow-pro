...
function latestReplyForRow(row: ReconciliationEmailRow) {
  const rawBody = String(row.body_text || row.body_html || "");
  return extractLatestReplyText(rawBody) || rawBody;
}

/**
 * Reconciles only the newest inbound customer message per sender. Older
 * terminal replies can therefore never close or suppress a customer who later
 * re-engaged. The same canonical latest-reply parser and classifier used by
 * the live path are reused here; no regex-based data repair is allowed.
 */
export function planInboundReplyReconciliation(rows: ReconciliationEmailRow[]) {
  ...
  for (const row of sorted) {
    ...
    const latestReply = latestReplyForRow(row);
    const classification = classifyInboundReply({ subject: row.subject, body: latestReply });
    ...
    candidates.push({ row, classification: classification.intent, latestReply });
  }
