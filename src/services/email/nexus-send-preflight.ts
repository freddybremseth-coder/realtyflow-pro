...
export function assessNexusSendPreflight(input: {
  brandId: string;
  profile: any;
  shortlist: any;
  presentation: any;
  ...
  const contactBrand = normalizedBrand(input.contact?.brand_id || input.contact?.brand);
  const brandId = normalizedBrand(input.brandId);
  const sameBrand = Boolean(contactBrand) && contactBrand === brandId;
  ...
  const checks = {
    profileApproved: isApproved(input.profile?.status),
    shortlistApproved: isApproved(input.shortlist?.status),
    presentationApproved: isApproved(input.presentation?.status),
    draftApproved: isApproved(input.draft?.status),
    draftUnsent,
    contactLinked: Boolean(input.contact?.id),
    sameBrand,
    recipientValid,
    suppressionClear,
    senderConfigured: input.senderConfigured,
    subjectPresent: Boolean(text(input.draft?.subject)),
    bodyPresent: Boolean(text(input.draft?.body_text)),
    clientReadyPropertyCount,
    propertyCount,
    verifiedPropertyLinkCount,
  };

  const blockers = unique([
    checks.profileApproved ? null : "Buyer Profile is not approved.",
    checks.shortlistApproved ? null : "Shortlist is not approved.",
    checks.presentationApproved ? null : "Presentation is not approved.",
    checks.draftApproved ? null : "Message draft is not approved.",
    checks.draftUnsent ? null : "Message draft is already sent, cancelled or otherwise closed.",
    checks.contactLinked ? null : "Buyer Profile has no linked CRM contact.",
    checks.sameBrand ? null : "CRM contact does not belong to the same brand.",
    checks.recipientValid ? null : "Recipient email is missing or invalid.",
    input.suppressionError ? "CRM suppression could not be verified; preflight fails closed." : null,
    input.suppressionBlocked ? "Recipient is suppressed or marked do-not-contact in CRM." : null,
    checks.senderConfigured ? null : "No active sender account is configured for this brand.",
    checks.subjectPresent ? null : "Approved message has no subject.",
    checks.bodyPresent ? null : "Approved message has no body text.",
    checks.clientReadyPropertyCount > 0 ? null : "No client-ready property remains in the approved shortlist.",
    checks.propertyCount > 0 ? null : "Presentation contains no properties.",
    checks.propertyCount > 0 && checks.verifiedPropertyLinkCount === checks.propertyCount ? null : "One or more presentation properties lack a verified public link.",
  ]);
  ...
  const warnings = unique([
    ...
    "Send permission is not granted by preflight. The same safety checks must run again immediately before any provider send.",
  ]);
...
export async function runNexusSendPreflight(input: {
...
}): Promise<NexusSendPreflightAssessment> {
  ...
  if (!profile || !shortlist || !presentation || !draft) {
    return assessNexusSendPreflight({
      brandId,
      profile,
      shortlist,
      presentation,
      draft,
      contact: null,
      shortlistItems: shortlistItemsResult.data || [],
      senderConfigured: Boolean(senderResult.data?.length),
      suppressionBlocked: false,
      suppressionError: "Required send-preflight dependency is missing.",
    });
  }

  const dependencyMismatch = String(shortlist.buyer_profile_id) !== buyerProfileId
    || String(presentation.buyer_profile_id) !== buyerProfileId
    || String(presentation.shortlist_id) !== shortlistId
    || String(draft.buyer_profile_id) !== buyerProfileId
    || String(draft.shortlist_id) !== shortlistId
    || String(draft.presentation_id) !== presentationId;
  if (dependencyMismatch) {
    return assessNexusSendPreflight({
      brandId,
      profile,
      shortlist: { ...shortlist, status: "mismatch" },
      presentation,
      draft,
      contact: null,
      shortlistItems: shortlistItemsResult.data || [],
      senderConfigured: Boolean(senderResult.data?.length),
      suppressionBlocked: false,
      suppressionError: "Send-preflight dependency mismatch.",
    });
  }
  ...
  const suppression = recipient
    ...
    : { blocked: false, blockedEmails: [] as string[] };
  ...
  return assessNexusSendPreflight({
    ...
    shortlistItems: shortlistItemsResult.data || [],
    senderConfigured: Boolean(senderResult.data?.length),
    suppressionBlocked: suppression.blocked,
    suppressionError: suppression.error || null,
  });
}
