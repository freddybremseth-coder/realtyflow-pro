...
const analysisOverviewPath = path.join(
  process.cwd(),
  "src/components/lead-intelligence/lead-intelligence-analysis-overview.tsx",
);
const criteriaReviewPath = path.join(
  process.cwd(),
...
const savedProfileContactPath = path.join(
  process.cwd(),
  "src/components/lead-intelligence/lead-intelligence-saved-profile-contact-panel.tsx",
);
const presentationHistoryPath = path.join(
  process.cwd(),
...
const shortlistDraftPanelPath = path.join(
  process.cwd(),
...
const propertyMatchDisplayPath = path.join(
  process.cwd(),
...
test("Lead Intelligence worklist is read-only and does not expose raw stored payloads", async () => {
  ...
  assert.equal(source.includes("leadIntelligenceMatchReturnUrl"), true);
  assert.equal(source.includes("leadIntelligenceMatchAnchor"), true);
  assert.equal(source.includes("lead-intelligence-match-"), true);
  assert.equal(source.includes("returnTo={presentationDraftReturnUrl}"), true);
  assert.equal(source.includes("returnTo={leadIntelligenceMatchReturnUrl(propertyMatchReturnBaseUrl, match.propertyId)}"), true);
  assert.equal(source.includes('if (!hash.startsWith("#lead-intelligence-match-")) return;'), true);
  assert.equal(source.includes("setHighlightedMatchId(propertyId)"), true);
  assert.equal(source.includes('target.scrollIntoView({ behavior: "smooth", block: "center" });'), true);
  assert.equal(source.includes("PropertyMatchThumbnail"), true);
  assert.equal(source.includes("matchPropertyImageUrl"), true);
  assert.equal(source.includes("match.property.imageUrl"), true);
  assert.equal(source.includes("match.property.gallery?.[0]"), true);
  assert.equal(source.includes("Building2"), true);
  assert.equal(source.includes("scroll-mt-28"), true);
  assert.equal(source.includes("ring-2 ring-primary-300/70"), true);
...
test("Inventory can open a property detail modal from Lead Intelligence internal links", async () => {
  ...
  assert.equal(source.includes("`${url.pathname}${url.search}${url.hash}`"), true);
