...
test("Lead Intelligence worklist is read-only and does not expose raw stored payloads", async () => {
  ...
  assert.equal(source.includes("PresentationManualSharingGate"), true);
  ...
  assert.equal(source.includes("Freddy-godkjenning før manuell deling"), true);
  assert.equal(source.includes("Kopier manuell sendepakke"), true);
  assert.equal(source.includes("Den lagrer ikke approval-status og sender ikke e-post."), true);
  assert.equal(source.includes("Lokalt klar"), true);
