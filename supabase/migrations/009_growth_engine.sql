...
-- ========================================
-- 5. Command Executions (AI Kommandosenter)
-- ========================================
...
CREATE INDEX IF NOT EXISTS idx_command_executions_date ON command_executions(created_at DESC);
...
DO $$ BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow all on command_executions') THEN
    CREATE POLICY "Allow all on command_executions" ON command_executions FOR ALL USING (true) WITH CHECK (true);
  END IF;
END $$;
