-- Ensure Growth OS learning consumes Instagram reactions/comments already present in canonical observed metadata.
-- Business outcomes remain weighted more heavily by the Learning Engine; this only prevents likes/comments from being silently dropped.
...
-- Backfill current canonical snapshots so the next learning refresh uses the complete engagement signal immediately.
